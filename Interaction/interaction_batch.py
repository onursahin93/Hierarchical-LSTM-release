...
class Interaction(BaseInteraction):
    ...
    def interact(self, model, data, params, trial=None):
        if params.is_train:
            test_loss_mean = self.interact_train(model, data, params, trial=trial)
            return test_loss_mean

    def interact_train(self, model, data, params, trial=None):
        ...
        for epoch_idx in trange(1, params.num_epoch + 1):
            ...
            if epoch_idx % self.evaluation_interval == 0:
                _, test_loss_mean = self.interact_test(model, data, params, epoch_idx)

            if (not trial is None) and (epoch_idx % 5 == 0):
                intermediate_value = test_loss_mean
                trial.report(intermediate_value, epoch_idx)
    ...
    def interact_test(self, model, data, params, epoch_idx):
        ...
        return epoch_loss_test, epoch_loss_test/sum_y_mask_test
