...
class Bank32NHDataReader(BaseDataReader):
    def __init__(self,
                 params):
        super(Bank32NHDataReader, self).__init__(params=params)
        self.device = params.device
        self.data_path = f"{MAIN_DIR}/Data/old_data/Bank/Bank32nh/bank32nh.data"
        self.test_data_path = f"{MAIN_DIR}/Data/old_data/Bank/Bank32nh/bank32nh.test"
        self.data_full_path = f"{MAIN_DIR}/Data/old_data/Bank/Bank32nh/bank32nh.txt"
        self.label_index = 32
        ...
        self.raw = torch.from_numpy(np.genfromtxt(self.data_full_path))
        self.raw_test = torch.from_numpy(np.genfromtxt(self.test_data_path))
        self.raw = torch.cat([self.raw, self.raw_test]) # just to keep the pipeline same, i split train and tests
        if self.is_normalize:
            self.normalize()
        self.train_ratio = params.train_ratio
        self.missing_ratio = params.missing_ratio

        self.train_length = 8192#round((self.raw.shape[0]*self.train_ratio))
        self.test_length = self.raw.shape[0] - self.train_length - self.n_step

        self.x_t_train = self.raw[:self.train_length,:self.label_index ]
        self.x_t_test = self.raw[self.train_length:-self.n_step, :self.label_index ] if self.n_step > 0 else self.raw[self.train_length:, :self.label_index ]
        self.y_t_train = (self.raw[self.n_step:self.train_length + self.n_step, self.label_index ]).unsqueeze(1)
        self.y_t_test = (self.raw[self.train_length + self.n_step:, self.label_index ]).unsqueeze(1)
...
if __name__ == "__main__":
    params = Namespace(**{"train_ratio": 0.6,
                          ...
                          "is_batch": False,
                          "is_normalize": True})
    reader = Bank32NHDataReader(params=params)
