...
class AirQualityUCIDataReader(BaseDataReader):
    def __init__(self,
                 params):
        super(AirQualityUCIDataReader, self).__init__(params=params)
        self.device = params.device
        self.data_path = f"{MAIN_DIR}/Data/old_data/AirQualityUCI/AirQualityUCI.csv"
        self.n_step = 1
        self.require_presence_pattern = False
        aa = np.loadtxt(self.data_path, dtype=np.str, delimiter=';')
        aa_b = np.char.replace(aa, ',', '.')[1:9358, 2:15]
        self.raw = torch.from_numpy(aa_b.astype(np.float32))
        ...
        self.x_t_train = self.raw[:self.train_length]
        self.x_t_test = self.raw[self.train_length:-self.n_step]
        self.y_t_train = self.raw[self.n_step:self.train_length + self.n_step]
        self.y_t_test = self.raw[self.train_length + self.n_step:]
        ...
        self.x_mask_test = self.raw_mask[self.train_length:-self.n_step]
...
if __name__ == "__main__":
    params = Namespace(**{"train_ratio": 0.6,
                          ...
                          "is_batch":False,
                          "is_normalize":True})
    reader = AirQualityUCIDataReader(params=params)
