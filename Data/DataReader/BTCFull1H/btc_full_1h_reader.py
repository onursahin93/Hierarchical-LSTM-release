...
class BTCDataReader(BaseDataReader):
    def __init__(self,
                 params):
        super(BTCDataReader, self).__init__(params=params)
        self.device = params.device
        self.data_path = f"{MAIN_DIR}/Data/old_data/BTC_FULL_1H/btcfull.csv"
        self.n_step = params.n_step
        self.require_presence_pattern = False
        # ratio = np.loadtxt(self.data_path)
        # self.raw = torch.from_numpy(ratio.cumprod(axis=0))
        data = np.loadtxt(self.data_path, delimiter=",")
        max_data = np.max(data)
        raw = data / max_data
        self.raw = torch.from_numpy(raw[:, 2]).unsqueeze(1)
...
if __name__ == "__main__":
    params = Namespace(**{"train_ratio": 0.6,
                          ...
                          "is_batch": False,
                          "is_normalize": True})
    reader = BTCDataReader(params=params)
